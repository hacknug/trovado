    ...
    <BaseFormFieldset>
      ...
      <div class="flex items-center justify-between mt-6">
        <div class="flex items-center">
          <input id="remember_me" type="checkbox" class="form-checkbox w-4 h-4 text-blue-600 transition duration-150 ease-in-out" checked />
        ...
        <div v-if="event === 'Sign In'" class="text-sm leading-5">
          <button class="hover:text-blue-500 focus:outline-none focus:underline font-medium text-blue-600 transition duration-150 ease-in-out" type="button" @click="$emit('resetPassword', userDetails)">Forgot your password?</button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-3 gap-3 mt-6 opacity-50 cursor-not-allowed">
        <div>
          <span class="inline-flex w-full rounded-md shadow-sm">
            <button type="button" class="hover:text-gray-400 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue inline-flex justify-center w-full px-4 py-2 text-sm font-medium leading-5 text-gray-500 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md" :class="`pointer-events-none`" disabled>
