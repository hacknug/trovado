<template>
  <div class="flex flex-col self-start overflow-hidden rounded-lg shadow-lg">
    ...
    <div class="sm:flex-row flex flex-col items-start justify-between flex-1 p-6 bg-white">
      <div class="flex-1">
        <p class="-mt-2 -ml-2 text-sm font-medium leading-5 text-blue-600">
        ...
        <g-link :to="shop.path" class="block mt-2">
        ...
        <div class="relative flex items-end justify-between w-full mt-4">
          <dl>
          ...
          <div class="owl:mt-2 absolute bottom-0 right-0 text-center">
            <span class="text-sm text-blue-500">Have you been here recently?</span>
            <span class="sm:w-auto flex w-full rounded-md shadow-sm">
    ...
    <div v-if="open" class="bg-gray-50 sm:px-6 owl:mt-6 flex flex-col justify-between px-4 py-6 border-t-2 border-gray-100">
...
<script>
import { BarChart2Icon, ChevronDownIcon } from 'vue-feather-icons'

export default {
  name: 'ShopCardAlt',
  components: { BarChart2Icon, ChevronDownIcon },
  props: {
    shop: {
      type: Object,
      required: true,
